...
// COUNT DATA
// ============================================================
$total_anggota = mysqli_fetch_row(
    mysqli_query($conn, "SELECT COUNT(*) FROM anggota WHERE deleted_at IS NULL")
)[0];

$total_periode = mysqli_fetch_row(
    mysqli_query($conn, "SELECT COUNT(*) FROM periode")
)[0];

$total_proker = mysqli_fetch_row(
    mysqli_query($conn, "SELECT COUNT(*) FROM proker")
)[0];

$total_log = mysqli_fetch_row(
    mysqli_query($conn, "SELECT COUNT(*) FROM log_aktivitas")
)[0];

// ============================================================
// PERIODE AKTIF (FIX: pakai label, bukan tahun_periode)
// ============================================================
$periode_aktif = mysqli_fetch_assoc(
    mysqli_query($conn, "
        SELECT label, tahun_mulai, tahun_selesai, status
        FROM periode
...
<div class="main">
  ...
  <div class="welcome">
    ...
    <p>
      Login sebagai <strong><?= htmlspecialchars($ses_role) ?></strong>
      · Periode Aktif:
      <strong>
        <?= $periode_aktif['label'] ?? ($periode_aktif['tahun_mulai'].'/'.$periode_aktif['tahun_selesai']) ?? '-' ?>
      </strong>
    </p>
  </div>
  ...
  <div class="quick-links">
    ...
    <a href="<?= tab_url('pages/anggota_tampil.php') ?>" class="qlink">
      ...
      <div>
        ...
        <span>Lihat daftar semua anggota</span>
      </div>
    </a>
    ...
    <a href="logout.php?tsid=<?= urlencode($tsid) ?>" class="qlink">
      ...
      <div>
        <strong>Logout Tab Ini</strong>
        <span>Keluar dari sesi tab ini</span>
      </div>
    </a>
